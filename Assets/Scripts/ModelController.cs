...
public class ModelController : MonoBehaviour, IModelViewer
{
    ...
    {
        if (allParts.TryGetValue(partID, out GameObject serverPartToShake)) { if (!shakingCoroutines.ContainsKey(serverPartToShake)) { originalLocalRotations[serverPartToShake] = serverPartToShake.transform.localRotation; shakingCoroutines[serverPartToShake] = StartCoroutine(ShakeCoroutine(serverPartToShake)); ShowLocalServerIcon(receivedWorldPosition, destroyIconSprite); } }
    }
    ...
    private void ShowLocalServerIcon(Vector3 worldPos, Sprite icon)
    {
        ...
        feedbackIconImage.sprite = icon; feedbackIconImage.rectTransform.anchoredPosition = localPoint; feedbackIconImage.gameObject.SetActive(true);
    }
