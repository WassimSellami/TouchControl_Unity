...
public class ModelController : MonoBehaviour, IModelViewer
{
    ...
    {
        if (axesContainer == null || serverCamera == null)
            return;
    ...
    public void RemoveModel(string modelID)
    {
        if (availableModels == null)
            return;
        ModelData modelToRemove = availableModels.Find(m => m != null && m.modelID == modelID);
        ...
        if (modelToRemove != null)
        {
            ...
            SaveRegistry();
            if (wsManager != null)
                wsManager.BroadcastModelList();
            ServerModelUIPanel serverUI = FindObjectOfType<ServerModelUIPanel>();
            if (serverUI != null)
                serverUI.PopulateServerList(GetAllModelsMetadata().models.ToList());
        }
    }
    ...
    private void ApplyDensityToPart(GameObject part)
    {
        ...
        foreach (Renderer rend in renderers)
        {
            if (rend.gameObject.name.Contains("Shaft") || rend.gameObject.name.Contains("Head"))
                continue;
            if (rend.material.HasProperty("_MinVal"))
                rend.material.SetFloat("_MinVal", volumeMinVal);
            if (rend.material.HasProperty("_MaxVal"))
                rend.material.SetFloat("_MaxVal", volumeMaxVal);
        }
    }
    ...
    private void RefreshModelLookup()
    {
        ...
        foreach (ModelData modelData in availableModels)
        {
    ...
    private IEnumerator LoadModelRoutine(string modelId)
    {
        ServerModelUIPanel serverUI = FindObjectOfType<ServerModelUIPanel>();
        if (serverUI != null)
            serverUI.SetListVisibility(false);
        ...
        if (loadingUI != null)
            loadingUI.Show(modelData.displayName);
    ...
    public void UnloadCurrentModel()
    {
        ...
        foreach (Coroutine coroutine in shakingCoroutines.Values)
            if (coroutine != null)
    ...
    {
        public string id; public string name; public string desc; public string filePath; public string size; public string thumbPath; public bool isVolumetric; public int dx, dy, dz;
    }
    [Serializable]
    public class RegistryWrapper
    {
        public List<ModelRegistryEntry> entries = new List<ModelRegistryEntry>();
    }
    ...
    private void SaveRegistry()
    {
        ...
        foreach (ModelData m in availableModels)
        {
            string path = "";
            bool isVol = false;
            int x = 0, y = 0, z = 0;
            if (m is PolygonalModelData p)
                path = p.modelFilePath;
            else if (m is VolumetricModelData v)
            {
                path = v.rawFilePath;
                isVol = true;
                x = v.dimX;
    ...
    private void LoadRegistry()
    {
        ...
        try
        {
            ...
            foreach (ModelRegistryEntry e in wrapper.entries)
            {
                ...
                if (e.isVolumetric)
                {
                    ...
                    v.description = e.desc;
                    v.rawFilePath = e.filePath;
                    v.dimX = e.dx;
                    v.dimY = e.dy;
                    v.dimZ = e.dz;
                    v.fileSize = e.size;
                    v.thumbnail = loadedThumb;
    ...
    {
        if (!modelDataLookup.TryGetValue(modelId, out ModelData data))
            return;
        ...
        if (proxyData != null && wsManager != null)
            wsManager.BroadcastCustomCommand("LOAD_PROXY_MESH", JsonUtility.ToJson(proxyData));
    }

    public ModelMetadataList GetAllModelsMetadata()
    {
        List<ModelMetadata> metadataList = new List<ModelMetadata>();
        foreach (ModelData modelData in availableModels)
        {
            if (modelData == null)
                continue;
    ...
    public void ExecuteSlice(SliceActionData data)
    {
        while (redoStack.Count > 0)
            CleanUpAction(redoStack.Pop());
        ActionRecord record = new ActionRecord { Type = ActionType.Slice, ActionID = data.actionID, Originals = new List<GameObject>(), NewHulls = new List<GameObject>() };
        foreach (string partID in data.targetPartIDs)
        {
            if (allParts.TryGetValue(partID, out GameObject originalPart) && originalPart.activeInHierarchy)
            {
                if (originalPart.GetComponentInChildren<VolumeRenderedObject>() != null)
                    ExecuteVolumetricSlice(originalPart, data, record);
                else
                    ExecuteMeshSlice(originalPart, data, record);
            }
        }
    ...
    public void RedoLastAction()
    {
        ...
        {
            foreach (GameObject hull in record.NewHulls)
                if (hull != null)
                    hull.SetActive(true);
            foreach (GameObject part in record.Originals)
                if (part != null)
                    part.SetActive(false);
        }
        else if (record.Type == ActionType.Destroy && record.DestroyedPart != null)
            record.DestroyedPart.SetActive(false);
        undoStack.Push(record);
    }
    ...
    public void ResetCrop()
    {
        ...
        for (int i = modelContainer.transform.childCount - 1; i >= 0; i--)
        {
            ...
            if (rootModel != null && child == rootModel)
                continue;
            Destroy(child);
        }
        if (rootModel != null)
    ...
    private void ClearCurrentModelAxisVisuals()
    {
    ...
    private IEnumerator AnimateSeparation(GameObject upperHull, GameObject lowerHull, GameObject original, Vector3 planeNormal, float separationFactor)
    {
        ...
        Vector3 separationVector = planeNormal * (originalBounds.size.magnitude * separationFactor * 0.5f);
        ...
        if (upperHull != null)
            upperHull.transform.position = upperEnd;
        if (lowerHull != null)
            lowerHull.transform.position = lowerEnd;
    }
    ...
    private void ExecuteVolumetricSlice(GameObject originalPart, SliceActionData data, ActionRecord record)
    {
        ...
        ApplyDensityToPart(partA);
        ApplyDensityToPart(partB);
        if (!allParts.ContainsKey(partA.name))
            allParts.Add(partA.name, partA);
        if (!allParts.ContainsKey(partB.name))
            allParts.Add(partB.name, partB);
    ...
    private void ExecuteMeshSlice(GameObject originalPart, SliceActionData data, ActionRecord record)
    {
        ...
        while (redoStack.Count > 0)
            CleanUpAction(redoStack.Pop());
        SliceUtility.SliceResult result = SliceUtility.ExecuteMeshSlice(meshTarget, data.planePoint, data.planeNormal, crossSectionMaterial, this, originalPart.transform.parent);
        if (result.isValid)
        {
            result.upperHull.name = originalPart.name + "_A";
            result.lowerHull.name = originalPart.name + "_B";
            if (allParts.ContainsKey(result.upperHull.name))
                allParts[result.upperHull.name] = result.upperHull;
            else
                allParts.Add(result.upperHull.name, result.upperHull);
            if (allParts.ContainsKey(result.lowerHull.name))
                allParts[result.lowerHull.name] = result.lowerHull;
            else
    ...
    private void SetupVisualHelpers()
    {
        ...
        Shader axisShader = Shader.Find("Hidden/Internal-Colored");
        if (axisShader == null)
            axisShader = Shader.Find("Standard");
    ...
    {
        if (feedbackIconImage == null || serverCamera == null || uiCanvasRectTransform == null)
            return;
    ...
    {
        public string ActionID; public GameObject DestroyedPart; public List<GameObject> NewHulls; public List<GameObject> Originals; public ActionType Type;
    }
}