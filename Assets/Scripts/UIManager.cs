...
public class UIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (redoButton != null)
            redoButton.onClick.AddListener(OnRedoButtonPressed);
        if (infoButton != null)
            infoButton.onClick.AddListener(ToggleInfoPanel);
        if (toggleAxesButton != null)
            toggleAxesButton.onClick.AddListener(OnToggleAxesPressed);

        if (toggleDensityButton != null)
            toggleDensityButton.onClick.AddListener(OnToggleDensityPressed);
        if (densityControlPanel != null)
            densityControlPanel.SetActive(false);

        if (minDensitySlider != null)
            minDensitySlider.onValueChanged.AddListener(OnDensityChanged);
        if (maxDensitySlider != null)
            maxDensitySlider.onValueChanged.AddListener(OnDensityChanged);
    }
    ...
    private void OnToggleDensityPressed()
    {
        if (densityControlPanel != null)
        {
            ...
            if (inputManagerRef != null)
                inputManagerRef.SetInteractionEnabled(!isActive);
        }
    }
    ...
    public void PopulateModelButtons(List<ModelMetadata> models, WebSocketClientManager wsManagerRef)
    {
        ...
        foreach (ModelMetadata meta in models)
        {
            ...
            DraggableModelIcon dragComp = btn.AddComponent<DraggableModelIcon>();
    ...
    private void OnToggleAxesPressed()
    {
        ...
        if (wsManager != null)
            wsManager.SendToggleAxes(currentAxesState);
    }
    ...
    public void ShowModelViewPanel()
    {
        ...
        if (densityControlPanel != null)
            densityControlPanel.SetActive(false);
        if (minDensitySlider != null)
            minDensitySlider.SetValueWithoutNotify(0f);
        if (maxDensitySlider != null)
            maxDensitySlider.SetValueWithoutNotify(1f);

        if (wsManager != null)
            wsManager.SendToggleAxes(true);
    }
